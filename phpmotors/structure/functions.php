...
function buildVehicleDetail($vehicle)
{
	...
	$html_str.=$message;
	$seconds=getSecondaryImages($vehicle['invId']);
	if(!empty($seconds))
	{
	$html_str.="<div id=\"car_thms\">\n";
	$html_str.="<h3 class=\"img-thumbs\">Alternate Images</h3>\n";
		foreach ($seconds as $thumb)
		{
			$html_str.="<img class=\"car_thm\" src='$thumb[invImage]' alt='Alternate Image of $vehicle[invMake] $vehicle[invModel] on phpmotors.com'>";
		}
	$html_str.="</div>\n";
	}
	...
	function buildImageDisplay($imageArray)
	{
		...
		foreach ($imageArray as $image)
		{
			...
			$id .= "<p><a href='?action=delete&imgId=$image[imgId]&filename=$image[imgName]' title='Delete the image'>Delete $image[imgName]</a></p>";
			$id .= '</li>';
		}
		$id .= '</ul>';
		return $id;
